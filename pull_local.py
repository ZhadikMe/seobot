#!/usr/bin/env python3
"""
pull_local.py — Local version of pull.py with reduced delays (for home IP, not Railway).

Usage:
    python pull_local.py <archive_url> <target_dir> [domain] [--seo-only]

Examples:
    python pull_local.py "https://web.archive.org/web/20230601/https://example.com/" D:/example
    python pull_local.py "https://web.archive.org/web/20230601/https://example.com/" D:/example example.com --seo-only

Differences from pull.py:
    - wget --wait=1 instead of --wait=3 (home IP not rate-limited)
    - No --random-wait
    - recover pause: 5s instead of 60s
    - recover per-asset sleep: 1s instead of 4s
    - Optionally runs SEO pipeline (--seo-only) after download
"""
...
def download_snapshot(archive_url: str, tmp_dir: str, domain_no_www: str, wget_host: str,
    ...
    base_flags = [
        ...
        '--tries=3',
        '--wait=1',           # local: 1s is fine, home IPs don't get rate-limited by archive.org
                              # (Railway/cloud IPs need --wait=3 --random-wait)
    ...
    # Asset time limit: ~2 sec/URL (--wait=1 local) + 5 min buffer, min 10 min
    if total_estimated:
        asset_limit_sec = max(600, int(total_estimated * 2) + 300)
...
def _recover_missing_assets(site_dir: str, domain_no_www: str, timestamp: str, scheme: str = 'http') -> None:
    ...
    # Brief pause before starting recovery
    print('  Пауза 5 сек перед recover...')
    time.sleep(5)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Local pull + optional SEO pipeline')
    parser.add_argument('archive_url', help='web.archive.org snapshot URL')
    parser.add_argument('target_dir',  help='Local directory to save site into')
    parser.add_argument('domain',      nargs='?', default='',
                        help='Domain override (e.g. example.com). Auto-detected if omitted.')
    parser.add_argument('--seo-only',  action='store_true',
                        help='Run SEO fixes after download (no translation)')
    args = parser.parse_args()

    site_dir = pull_snapshot(args.archive_url, args.target_dir)

    if args.seo_only:
        from run_local import run_pipeline
        from translate import detect_source_lang
        domain = args.domain or os.path.basename(site_dir.rstrip('/\\')) + '.com'
        print(f'\n{"="*60}')
        print(f'Running SEO pipeline on {site_dir} (domain={domain})')
        print(f'{"="*60}\n')
        run_pipeline(site_dir, domain, mode='seo_only', langs=[])
        print('\nDone. Now you can run translation separately via run_local.py --mode translate')
